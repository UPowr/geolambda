...
docker build --platform linux/amd64 . -t developmentseed/geolambda:${VERSION}
docker run --rm -v $PWD:/home/geolambda ${INTERACTIVE} -t developmentseed/geolambda:${VERSION} package.sh

cd python
docker build . -platform linux/amd64 --build-arg VERSION=${VERSION} -t developmentseed/geolambda:${VERSION}-python
